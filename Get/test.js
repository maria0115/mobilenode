...
function Authority(config, qObj, res, req) {
    console.log(qObj.cookie.length, "Authority")
    if (qObj.cookie.length == 0) {
        console.log("cookie 없니")
        util.writeSuccess({ UserName: "Anonymous" }, res);
        return;
    } else {
        console.log(qObj.cookie)
        return axios({
            method: "get",
...
async function login(config, qObj, res, req) {
    // console.log("여길안들어온다라")
    if (!qObj.data.uuid) {
        // 로그인 실패
        util.writeSuccess({ success: false, message: "uuid undefined" }, res);
        return;
    }
    try {
        requests.post({
            ...
            if (response && response.headers["set-cookie"]) {
                ...
                // 로그인 실패
                util.writeSuccess({ success: false, message: "서버에서 ltpa cookie 값을 set 해주지 않음" }, res);
            }
        })
    } catch (error) {
...
"uuid": "${qObj.data.uuid}"
    ...
    if (data.success) {
        ...
        if (cnt === 0) {
            ...
            if (!state) {
                return { success: false, message: "분실등록된 기기" };
...
"reader": "${qObj.data.reader}"
    ...
    if (data.success) {
        ...
        if (cnt < config.registerInterfaceLimitCnt) {
            ...
            return { success: false, message: "등록 기기 6개 이상" };
        }
    }
    return data;
}
...
async function getReaders(config, cookie) {
    var url = `${config.host + config.getReaders}`;
...
async function interfaceRegister(config, qObj, res, req) {
    ...
    const url = config.elastic_address.v7 + `/user_interface/_doc/${qObj.data.uuid}`;
