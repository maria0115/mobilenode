    ...
    try {
        requests.post({
            ...
            if (response && response.headers["set-cookie"]) {
                ...
                // 로그인 실패
                util.writeSuccess({ success: false, message: "fail", alert: true }, res);
            }
        })
    } catch (error) {
...
"query":{
    ...
    if (data.success) {
        ...
        if (cnt === 0) {
            ...
            if (!state) {
                return { success: false, message: "lost", alert: true };
...
"reader": "${qObj.data.reader}"
    ...
    if (data.success) {
        ...
        if (cnt < config.registerInterfaceLimitCnt) {
            ...
            //수정*
            return { success: false, message:'excess' , count: config.registerInterfaceLimitCnt,  alert: true };
        }
    }
    return data;
}
