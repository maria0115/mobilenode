...
async function getMyMailInfo(qObj) {
    ...
    var query = `{
        "query": {
          "match": {
            "notesId": "${qObj.readers.toUpperCase()}"
          }
        }
      }`;
    ...
    try {
        ...
        for (var sendToIdx = 0; sendToIdx < sendTo.length; sendToIdx++) {
            ...
            if (item["type"] == "u") {
                ...
                // 회사이름 추출
                var nameArr = item["fullname"].split("/");
                sendToObj.company = nameArr[2];
        ...
        for (var copyToIdx = 0; copyToIdx < copyTo.length; copyToIdx++) {
            ...
            if (item["type"] == "u") {
                ...
                if (item["company"] == null) {
                    copyToObj.company = item["fullname"].split("/")[2];
                } else {
