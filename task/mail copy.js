...
const get = async (config, qObj, res, req) => {
    ...
    //폴더안에 리스트 가져오기
        ...
        for (var a = 0; a < result.viewentry.length; a++) {
            ...
            try {
                if (result.viewentry[a].entrydata[7].number['0'] == "186") {
                    obj.tostuff = { "receive": false, "ref": true };
                } else if (result.viewentry[a].entrydata[7].number['0'] == "185") {
                    obj.tostuff = { "receive": true, "ref": false };
                } else if (result.viewentry[a].entrydata[7].number['0'] == "184") {
                    obj.tostuff = { "receive": true, "ref": false };
                } else if (result.viewentry[a].entrydata[7].number['97'] != '') {
                    obj.tostuff = { "receive": true, "ref": false };
                } else {
    ...
    //조직도 검색
        ...
        for (var orgInx = 0; orgInx < result.length; orgInx++) {
            ...
            // console.log(result[orgInx]["_source"]["sort"],"     정렬");
            orgArr[orgInx] = orgObj;
...
const post = async (config, qObj, res, req) => {
    ...
    if (qObj.type === "mailsend" || qObj.type === "draftSave" || qObj.type === "draft_edit") {
        ...
        */
        ...
        if (qObj.formdata.MailTypeOption == undefined) {
        ...
        if (qObj.formdata.docType == undefined) {
...
async function getMyMailInfo(qObj) {
    ...
    var myInfo = {};
    if (qObj.language == "ko") {
        myInfo.name = result["_source"].name["ko"] + " " + result["_source"].position["ko"];
        myInfo.shortname = result["_source"].name["ko"];
        myInfo.department = result["_source"].departmentname["ko"];
        myInfo.company = result["_source"].companyname["ko"];
    } else if (qObj.language == "en") {
        myInfo.name = result["_source"].name["en"] + " " + result["_source"].position["en"];
        myInfo.shortname = result["_source"].name["en"];
        myInfo.department = result["_source"].departmentname["en"];
        myInfo.company = result["_source"].companyname["en"];
    }
    myInfo.id = result["_source"]["@id"];
    myInfo.email = result["_source"]["email"];
    myInfo.parentcode = result["_source"]["departmentcode"];
    myInfo.companycode = result["_source"]["companycode"];
    myInfo.mycode = result["_source"]["empno"];
    myInfo.kinds = result["_source"]["@form"];
    myInfo.mobile = result["_source"]["mobile"];
    myInfo.office = result["_source"]["office"];
...
async function getDetail(qObj, res, req, url) {
    ...
    if (attachName != "") {
        ...
        for (var attachIdx = 0; attachIdx < attachName.length; attachIdx++) {
            ...
            attachObj.url = util.strRight(attachUrl[attachIdx], "mail.pcnieng.com");
...
async function unreadList(qObj, res, req, url) {
    ...
    for (var i = size * page; i < size * page + size; i++) {
        ...
        if (result["viewentry"][i] == undefined) {
            ...
            try {
                if (result.viewentry[i].entrydata[7].number['0'] == "186") {
                    obj.tostuff = { "receive": false, "ref": true };
                } else if (result.viewentry[i].entrydata[7].number['0'] == "185") {
                    obj.tostuff = { "receive": true, "ref": false };
                } else if (result.viewentry[i].entrydata[7].number['0'] == "184") {
                    obj.tostuff = { "receive": true, "ref": false };
                } else if (result.viewentry[i].entrydata[7].number['97'] != '') {
                    obj.tostuff = { "receive": true, "ref": false };
                } else {
                    console.log("ssssssssss");
                    obj.tostuff = { "receive": true, "ref": false };
