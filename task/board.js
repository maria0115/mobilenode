...
const get = async (config, qObj, res, req) => {
    ...
    if (qObj.type === 'notice') {
        ...
        if (qObj.boardType == "congratulate") {
            ...
            url = url.replace("#unid#", qObj.unid);
        } if (qObj.boardType == "news") {
            url = config.host + config.board.detail_news;
            url = url.replace("#unid#", qObj.unid);
        }
        ...
        switch (qObj.boardType) {
            ...
            case "news":
                url = config.host + config.board.search_news;
                qObj.search = `(([boardid]=bbs0001)) and (`; break;
            case "congratulate":
    ...
    } else if (qObj.type === "news") {
        var url = config.host + config.board.news;
        url = url.replace(/#size#/, qObj.size);
        url = url.replace(/#page#/, qObj.page);
        url = url.replace(/#category#/, qObj.lnbid);
        console.log(url);
...
const post = async (config, qObj, res, req) => {
    ...
    if (qObj.type === "write_congratulate") {
        ...
        await write(config, qObj, res, req, url);
    } else if (qObj.type === "write_news") {
        url = config.board.write_news;
        url = url.replace("#lnbid#", qObj.formdata.lnbid);
        await write(config, qObj, res, req, url);
        ...
        url = url.replace("#unid#", qObj.formdata.unid);
        await write(config, qObj, res, req, url);
    } else if (qObj.type === "editItem_news") {
        url = config.board.editItem_news;
        url = url.replace("#unid#", qObj.formdata.unid);
        await write(config, qObj, res, req, url);
    }
};
...
async function getDetail(config, qObj, res, req, url) {
    ...
    if (config.bodyurl) {
        ...
        } else if (qObj.boardType == "news") {
            url = config.board.detailBody_news;
        }
...
function likeIt(config, qObj, res, req) {
    ...
    if (qObj.boardType == "congratulate") {
        ...
        url = config.host + config.board.reply_news
        formdata.append("pardb_path", "/dwp/com/share/outside/newsclip.nsf");
    }
...
function deletedItem(config, qObj, res, req) {
    ...
    if (qObj.body.boardType == "congratulate") {
        ...
        formdata.append("actiontype", "del_temp");
    } else if (qObj.body.boardType == "news") {
        url = config.host + config.board.delete_news
        formdata.append("actiontype", "del_temp");
    }
...
async function getBoardSearch(qObj, res, req, url) {
    ...
    var search = "";
    let sqry = qObj.searchword;
    sqry = (isTwoByteCheck(sqry) ? sqry : "*" + sqry + "*");
    qObj.searchword = sqry;
