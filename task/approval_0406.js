...
const get = async (config, qObj, res, req) => {
    ...
    if (qObj.type === 'approving') {
        //결재 중 문서
        console.log(url, "000000000000000000000000000");
...
async function approving(config, qObj, res, req) {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        var sCurFullListArr = common.strToArr(row["_curusersfull"], ";", "^"); //현결재자
...
async function getItemList(config, qObj, res, req, url) {
    ...
    for (var resultIdx = 0; resultIdx < result.length; resultIdx++) {
        ...
        if (qObj.type == "draft") {
            var openurl = "/dwp/#comalias#/workflow/aprv/aprvstart.nsf/vdockey/#unid#?opendocument";
            openurl = openurl.replace("#unid#", result[resultIdx]["_unid"]);
            openurl = openurl.replace("#comalias#", qObj.userInfo.comalias);
...
//결재문서 상세보기
async function getDetailItem(config, qObj, res, req) {
    // console.log(qObj);
    ...
    displayDataObj.unid = $("input[name='DocKey']").val();
    ///////////////////////////////////////////////////////
...
async function agreeNreject(config, qObj, res, req) {
    ...
    agreeUrl = qObj.formdata.openurl.replace("opendocument", "EditDocument&editchk=false&Seq=1&formtype=");
...
async function write(config, qObj, res, req) {
    ...
    formdata.append("sReceiveOrgName", ""); // 다음 결재자 notesid  진성원/204010/SIS
    formdata.append("sReceiveOrgNameFull", ""); // 다음결재자 정보  S^ko:진성원,en:Jin SeongWon^204010^진성원/204010/SIS^K-SIS_300001^K-SIS_200001^ko:수석연구원,en:Principal Research Engineer^K-SIS_50130^ko:수석연구원,en:Principal Research Engineer^K-SIS_50130^K-SIS^ko:Domino파트,en:Domino Part^ko:새롬정보,en:Saerom^^
...
async function editItem(config, qObj, res, req) {
    ...
    // agreeUrl = agreeUrl.replace("#unid#", qObj.unid);
    agreeUrl = qObj.formdata.openurl.replace("opendocument", "EditDocument&Seq=1&popup=1");
